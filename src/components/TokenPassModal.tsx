...
export const TokenPassModal: React.FC<TokenPassModalProps> = ({ token, onClose }) => {
  ...
  // is going on with layout/scroll — this is the pass/QR view people
  // most need a guaranteed way out of.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="ticket-stub w-full max-w-lg shadow-2xl overflow-hidden relative max-h-[95vh] flex flex-col my-4"
      >
        {/* Pass Header */}
        <div className="p-6 bg-gradient-to-r from-agri-800 via-agri-900 to-slate-900 bg-grain text-white relative text-center">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-2 rounded-full bg-paper/10 hover:bg-paper/20 text-white transition cursor-pointer z-10"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        {/* Pass Printable Body */}
        <div className="p-6 overflow-y-auto space-y-5 flex-1 bg-paper">
          {/* Token Big Badge */}
          <div className="p-4 rounded-2xl bg-[#FFFDF8] border-2 border-dashed border-agri-400 shadow-sm text-center relative">
            <span className="stamp-badge absolute -top-4 left-1/2 -translate-x-1/2 bg-[#FFFDF8] text-agri-700 text-[10px] rotate-2">
              Official Token
            </span>
            ...
            {qrUrl && (
              <div className="my-3 flex justify-center">
                <img
                  src={qrUrl}
                  alt="Token QR Code"
                  className="w-44 h-44 rounded-xl border border-slate-200 p-1 bg-paper shadow-xs"
                />
              </div>
            )}
          ...
          {/* Details Table */}
          <div className="bg-[#FFFDF8] rounded-2xl p-4 border border-slate-200 space-y-3 text-xs">
        ...
        <div className="ticket-perforation p-4 sm:p-6 bg-slate-100 flex flex-wrap items-center justify-between gap-2">
          <button
          ...
          <button
            onClick={handlePrint}
            className="p-2.5 rounded-xl bg-paper border border-slate-300 hover:bg-slate-50 text-slate-700 transition cursor-pointer"
            title={t('printPass')}
          >
