...
class Database {
  ...
  load() {
    if (fs.existsSync(DATA_FILE)) {
      try {
        ...
        this.invalidatedTokens = saved.invalidatedTokens || [];
  ...
  invalidateToken(jti) {
    if (!jti) return;
    if (!this.invalidatedTokens) this.invalidatedTokens = [];
    if (!this.invalidatedTokens.includes(jti)) {
      this.invalidatedTokens.push(jti);
      this.persist();
    }
  }

  isTokenInvalidated(jti) {
    if (!jti || !this.invalidatedTokens) return false;
    return this.invalidatedTokens.includes(jti);
  }
